namespace WebTaskManagerEfDb.Controllers
{
    ...
    public class TasksManagerController : BaseController<TaskEntity, TasksEditVM, TasksListVM>
    {
        ...
        public ActionResult Details(int? id)
        {
            ...
            TaskEntity task = (id == null || id <= 0) ? new TaskEntity() : taskRepo.GetById(id);
            UserEntity creatorUser = userRepo.GetById(task.CreatorId);
            UserEntity responsibleUser = userRepo.GetById(task.ResponsibleUsers);
            ...
            //fill the pager with the specific data //Yanka beshe tuk :D :D :D kaza da go iztriq ama nqma

            string action = this.ControllerContext.RouteData.Values["action"].ToString();
            string controller = this.ControllerContext.RouteData.Values["controller"].ToString();
            
            model.PagerComments = new Pager(commentRepo.GetAll().Count(), model.PagerComments.CurrentPage, "PagerComments.", action, controller, model.PagerComments.PageSize);

            model.PagerWorkLog = new Pager(timeRepo.GetAll().Count(), model.PagerWorkLog.CurrentPage, "PagerWorkLog.", action, controller, model.PagerWorkLog.PageSize);
